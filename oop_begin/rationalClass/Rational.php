...
class Rational
{
	...
	// public function nok($rational)// находим наименьшее общее кратное (общий делитель) 
	// {
	// 	$result = 1;
	// 	while (($result % $this->getDenom()) != 0 || ($result % $rational->getDenom()) != 0)  {
	// 		$result++;
	// 	}
	// 	return $result;
	// }
	...
	{
		// $nok = $this->nok($rational);
		$totalDenom = ($this->getDenom() * $rational->getDenom());
		$totalNumer = ($this->getNumer()) * ($totalDenom / $this->getDenom()) + ($rational->getNumer()) * ($totalDenom / $rational->getDenom());

		return new Rational($totalNumer, $totalDenom);
	}
